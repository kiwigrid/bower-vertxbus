...
!function(factory) {
	...
	var EventBus = function(url, options) {
		...
		this.sockJSConn.onmessage = function(e) {
			...
			if (json.replyAddress) {
				replyHandler = function(message, callback) {
					// Send back reply
					self.send(json.replyAddress, message, callback);
				};
			}
	 ...
	 */
	EventBus.prototype.send = function(address, message, callback) {
		// are we ready?
	 ...
	 * @param {Object} message
	 */
	EventBus.prototype.publish = function(address, message) {
		// are we ready?
		if (this.state !== EventBus.OPEN) {
			throw new Error('INVALID_STATE_ERR');
		}
